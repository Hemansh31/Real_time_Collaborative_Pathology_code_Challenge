...
function initializeSocketConnection(){
    // $RTCP.socket = io("http://localhost:8000", {transports: ['websocket', 'polling', 'flashsocket']});
    $RTCP.socket = io('', {
        path: "/rtpc/socket.io/",
...
function addSocketEventListeners(){
    ...
    $RTCP.socket.on('newPeerAdd', (jsonObject) =>{
        ...
        const sendThis = {
            ...
            center : {
                x : $RTCP.viewer.viewport.viewportToImageCoordinates($RTCP.viewer.viewport.getCenter()).x,
                y : $RTCP.viewer.viewport.viewportToImageCoordinates($RTCP.viewer.viewport.getCenter()).y
            }           
        };
    ...
    $RTCP.socket.on('newPeerRevert', (jsonObject) =>{
        ...
        if(!$RTCP.initialState){
            $RTCP.initialState = true;
            $RTCP.panState = false;
            $RTCP.zoomState = false;
            var newCenter = $RTCP.viewer.viewport.imageToViewportCoordinates(parseFloat(payload.center.x), parseFloat(payload.center.y));
...
function loadSlides(index){
    ...
    if($RTCP.action === 'join'){
        $RTCP.initialState = false;
    } else{
        $RTCP.initialState = true;
    }

    $RTCP.viewer.addOnceHandler('tile-drawn', function(event){
        $RTCP.panState = true;
        $RTCP.zoomState = true;
    });
